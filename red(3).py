...
class BookInfo:
    ...
    table:int
    def __str__(self):
        return f"{self.table}|{self.name}|{self.phone}|{self.date}"
class Mybooking:
...
def Booking(Input:list):
    ...
    mybook.update()
    print(mybook)
    mybook.allocate()#try to allocate table(s) for the booking
    return None
...
if __name__ == '__main__':
    ...
    print(ls)
    Booking(ls)
    userInput = "Booking|Leo|2025-07-01|1344|4"
    ls:list= userInput.split('|')
    Booking(ls)
    userInput = "Booking|Leo|2025-06-01|1344|4"
    ls:list= userInput.split('|')
    Booking(ls)
    print(bookdict)
    Listing()
